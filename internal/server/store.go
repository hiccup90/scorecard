...
func scanCheckins(rows *sql.Rows) ([]Checkin, error) {
	...
	for rows.Next() {
		...
		if level.Valid { c.ReviewLevel = &level.String }
		if minutes.Valid { v := int(minutes.Int64); c.ReviewMinutes = &v }
		c.CountsForStreak = counts == 1
		if note.Valid { c.ReviewNote = note.String }
		if reviewedAt.Valid { c.ReviewedAt = &reviewedAt.String }
		if reverseReason.Valid { c.ReverseReason = reverseReason.String }
		out = append(out, c)
	}
	return out, rows.Err()
}
...
func award(mode string, base int, level string, minutes int) (int, string, string, int, error) {
	...
	case "quality":
		...
		if !ok { return 0, "", "", 0, errors.New("无效的质量档位") }
		return base * m, labels[level], level, 0, nil
	case "duration":
		if minutes < 10 { return 0, "", "", 0, errors.New("时长至少 10 分钟") }
		units := minutes / 10
...
func recalcStreakTx(tx *sql.Tx, userID, activityID int64) (int, error) {
	...
	for rows.Next() {
		var raw string
		if err := rows.Scan(&raw); err != nil { return 0, err }
...
func scanRewards(rows *sql.Rows) ([]Reward, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.Name, &r.Cost, &r.Description, &r.Stock, &auto, &enabled); err != nil { return nil, err }
...
func scanRedemptions(rows *sql.Rows) ([]Redemption, error) {
	...
	for rows.Next() {
		var r Redemption
		if err := rows.Scan(&r.ID, &r.UserID, &r.UserName, &r.RewardID, &r.RewardName, &r.CostAtTime, &r.Status, &r.CreatedAt, &r.ReviewedAt, &r.ReviewNote); err != nil { return nil, err }
		out = append(out, r)
	}
	return out, rows.Err()
}
